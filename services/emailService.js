...
const sendApplicationSubmittedEmail = async (application) => {
  ...
  const content = `
    ...
    <div class="info-box">
      ...
      <p><span class="info-label">Registration Fee:</span> <span class="info-value">₹${application.payment.amount || 1000}</span></p>
...
const sendPaymentReceivedCredentialsEmail = async (application, temporaryPassword) => {
  ...
  const content = `
    ...
    <p>We are pleased to inform you that your payment of <strong>₹${application.payment.amount || 1000}</strong> has been <strong>verified and received successfully</strong>.</p>

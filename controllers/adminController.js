...
const getDashboardStats = async (req, res, next) => {
  try {
    ...
    const totalRevenue = verifiedApplications.reduce(
      (acc, app) => acc + (app.payment?.amount !== undefined ? app.payment.amount : 1499),
      0
    );
...
const createAdminApplication = async (req, res, next) => {
  try {
    ...
    const originalAmount = Number(referral?.originalAmount || req.body.originalAmount || 1499);
...
const adjustCandidateDiscount = async (req, res, next) => {
  try {
    ...
    const originalBaseFee = application.referral?.originalAmount || 1499;
